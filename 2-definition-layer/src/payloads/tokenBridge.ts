...
const fixedLengthStringItem = {
  ...
  custom: {
    to: (val: Uint8Array) =>
      range(val.byteLength).map((i) => String.fromCharCode(val[i])).join(""),
    from: (str: string) =>
...
const payloadIdItem = <ID extends number>(id: ID) => (
  { name: "payloadId", binary: "uint", size: 1, custom: id, omit: true }
...
declare global { namespace Wormhole {
  interface PayloadLiteralToDescriptionMapping extends ToMapping<typeof tokenBridgePayloads> {}
}}
